...
1. **App Replica 模式（竞品逆向复刻）**：以竞品真机截图/录屏为视觉与交互基准，通过逆向分析提取设计 Token 与层级，进行高保真像素级复刻。
2. **PRD & Prototype 模式（需求原型驱动开发）**：以产品需求文档（PRD/功能说明）与高保真原型图（Figma/Sketch 导出的标准设计切图）为输入，完成业务架构、SwiftUI 原生 UI、数据流与业务用例实现，实现逻辑与视觉双重闭环自愈。
...
### PRD & Prototype（需求原型驱动开发）

针对“需求文档 + 原型图”驱动的任务，验收标准涵盖功能逻辑与设计稿对齐：

* **业务逻辑断言（XCTest）**：必须覆盖 PRD 声明的核心业务规则、边界校验、状态变更与持久化逻辑，不能仅凭 UI 存在判断完成。
* **Mock 数据对齐**：为确保视觉探针度量（Ink Core 与排版）不被动态数据干扰，SwiftUI 页面必须支持注入与原型切图一致的 Preview / Mock 数据集。
* **画板与安全区规整**：设计原型切图必须统一为 `@3x` 标准尺寸；设计稿未绘制系统状态栏时，视觉探针应避开顶部 Safe Area 或应用 Status Bar Mask，避免假阳性色差。
* **页面路由直达**：多页面原型任务必须支持通过路由参数（如 Launch Arguments 或 AppState）直达指定页面再截图对比。
