...
program
  ...
  .action(async (options) => {
    ...
    // 3. Single-Task Mode (Preserves backward compatibility)
    if (!options.project) {
...
program
  ...
  .action(async (options) => {
    ...
    console.log('🔮 [AIRE Plan] Starting Upstream Multi-Agent Analysis Pipeline (Antigravity CLI)...');
    console.log(`📁 Project: ${projectPath}`);
    ...
    try {
      const isMock = process.env.AIRE_MOCK_RUNNER === '1' || process.env.AIRE_MOCK_RUNNER === 'true';
      const agentModel = isMock ? new SmartHeuristicAgentModel() : new AntigravityAgentModel();
      const pipeline = new AnalysisPipeline({ agentModel });
      const orchestrator = new PlannerOrchestrator({ pipeline });
      ...
      if (options.autoRun) {
        console.log('\n🚀 [AIRE Plan] --auto-run enabled. Checking workspace safety boundaries...');

        // Verify git workspace cleanliness: ensure NO user modifications are committed
        try {
          ...
          if (gitStatus.trim().length > 0) {
            ...
            const dirtyFiles = dirtyLines.map((l) => l.slice(2).trim());

            // Whitelist of files generated by planner
            const plannerArtifacts = ['task-graph.yaml', '.aire', 'docs'];
            const userDirtyFiles = dirtyFiles.filter((f) => {
              return !plannerArtifacts.some((art) => f.startsWith(art));
            });

            if (userDirtyFiles.length > 0) {
              console.error('\n❌ [AIRE Plan] Workspace Safety Violation:');
              console.error('Working directory contains modified or untracked user files:');
              for (const uf of userDirtyFiles) {
                console.error(`   - ${uf}`);
              }
              console.error('\nRefusing to run --auto-run to prevent accidental commits of your changes.');
              console.error('Please commit or stash your modifications first, or execute manually:');
              console.error(`   aire run --task-graph ${planResult.outputPath}\n`);
              process.exit(1);
            }

            // Only stage and commit planner artifacts! NEVER git add -A!
            await execFileAsync('git', ['add', '--', 'task-graph.yaml', '.aire', 'docs'], { cwd: projectPath });
            await execFileAsync('git', ['commit', '-m', 'chore(plan): save generated task graph and specifications'], { cwd: projectPath });
          }
        } catch (err: any) {
          if (err.message && err.message.includes('Workspace Safety Violation')) {
            throw err;
          }
