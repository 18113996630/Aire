...
### 2.3 原型设计切图（Figma / Sketch）规整与前置预处理
针对 **PRD & Prototype（需求与原型驱动开发）** 场景，输入基准为设计稿导出的切图，必须遵循以下规整与预处理规则：
1. **标准画板尺寸与比例约束**：
   - 原型切图必须严格按照目标真机物理分辨率导出（例如 iPhone 15/16 Pro 的 `@3x` 规格 `1179 × 2556 px`，对应 `393 × 852 pt`）；
   - 严禁随意拉伸或非标准画板导出，宽高比误差必须 $< 1\%$。
2. **系统状态栏与安全区遮罩（Status Bar Masking）**：
   - Figma 原型图通常仅绘制 App 业务 View，未绘制 iOS 系统状态栏（时间、信号、电池等）；
   - 视觉评估引擎在批处理探针时，自动为原型图应用 **Status Bar Mask**（默认屏蔽 `y < 59pt` 顶部区域），避免将系统组件差异误判为视觉缺陷。
3. **Mock 数据注入对齐（Mock Data Injection for Ink Core）**：
   - 原型图上的文案、数值多为静态示例（如“本月支出 ¥3,280.00”）；
   - SwiftUI 开发时必须为界面提供与原型完全一致的 Mock 数据集，确保文字排版、行高（Bands）与墨水核（Ink Core）比对在同等内容基准下运行，避免因内容动态变化引入虚假视觉缺陷。
4. **多页面路由直达（Route-based Screen Targeting）**：
   - 非首页原型图必须关联路由标识（通过 `routes.yaml` 声明）；
   - 模拟器运行时通过环境变量或启动参数（如 `-startRoute /editor`）直达目标 View 后再截屏比对，确保单页面独立闭环。
